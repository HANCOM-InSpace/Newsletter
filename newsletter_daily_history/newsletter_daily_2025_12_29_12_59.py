...
# 1차 후보 개수 (토픽당 raw 수집)
ARTICLES_PER_TOPIC_RAW = 40

# 언어별 목표 개수 (한글 30% : 영어 70%)  → raw 40 기준
ARTICLES_PER_LANG_KO = 12   # 30%
ARTICLES_PER_LANG_EN = 28   # 70%
...
YEAR_FOLDER  = f"{now_kst.year}daily"        # 예: "2025daily"
MONTH_FOLDER = f"{now_kst.month:02d}daily"   # 예: "12daily"
DAY_FOLDER   = f"{now_kst.day:02d}"          # 예: "29"

FOLDER_PATH = f"{YEAR_FOLDER}/{MONTH_FOLDER}/{DAY_FOLDER}"
# 예: "2025daily/12daily/29"
...
<title>한컴인스페이스 {DAILY_LABEL} 뉴스레터</title>
...
# ============================================================
# Colab 전용 (DAILY):
# notebook -> newsletter_daily.py 업데이트
# + newsletter_daily_history/에 스냅샷 저장 (weekly와 완전 분리)
# ============================================================
...
if _in_colab():
    ...
    payload = {
        "message": "Update newsletter_daily.py from Colab notebook",
        "content": content_b64,
        "branch": BRANCH,
    }
    if sha:
    ...
    # ---- DAILY HISTORY (완전 분리) ----
    HISTORY_DIR = "newsletter_daily_history"

    KST = timezone(timedelta(hours=9))
    ts = datetime.now(KST).strftime("%Y_%m_%d_%H_%M")
    history_path = f"{HISTORY_DIR}/newsletter_daily_{ts}.py"
    api_url_hist = f"https://api.github.com/repos/{GITHUB_OWNER}/{GITHUB_REPO}/contents/{history_path}"

    payload_hist = {
        "message": f"Archive snapshot (daily): {history_path}",
        "content": content_b64,
        "branch": BRANCH,
    }
    ...
    if resp.status_code in (200, 201):
        print(f"✅ DAILY 히스토리 저장 완료: {history_path}")
    else:
        ts2 = datetime.now(KST).strftime("%Y_%m_%d_%H_%M_%S")
        history_path2 = f"{HISTORY_DIR}/newsletter_daily_{ts2}.py"
        api_url_hist2 = f"https://api.github.com/repos/{GITHUB_OWNER}/{GITHUB_REPO}/contents/{history_path2}"
        payload_hist["message"] = f"Archive snapshot (daily): {history_path2}"
        resp2 = requests.put(api_url_hist2, headers=headers, json=payload_hist)
        if resp2.status_code in (200, 201):
            print(f"✅ DAILY 히스토리 저장 완료(초 포함): {history_path2}")
        else:
            raise RuntimeError(f"DAILY 히스토리 저장 실패: {resp2.status_code} / {resp2.text}")
