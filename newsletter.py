...
# ============================================================
# 11. (Colab 전용) newsletter.py 자동 버전 업로드 + 최신본 갱신
# ============================================================
if IN_COLAB:
    try:
        # 1) 현재 Colab 런타임에서 실행 중인 스크립트 파일 이름
        #    (Colab에서 %%writefile newsletter.py 로 저장했다고 가정)
        SCRIPT_FILENAME = "newsletter.py"

        # 2) KST 기준 타임스탬프 생성
        KST = timezone(timedelta(hours=9))
        now_kst = datetime.now(KST)
        timestamp = now_kst.strftime("%Y_%m_%d_%H_%M")  # 예: 2025_12_12_13_50

        # 3) 버전 이름 생성
        versioned_filename = f"newsletter_{timestamp}.py"
        versioned_repo_path = f"versions/{versioned_filename}"  # 원하면 폴더 이름 변경 가능

        # 4) 현재 Colab에 있는 최신 코드 읽기
        with open(SCRIPT_FILENAME, "r", encoding="utf-8") as f:
            script_content = f.read()

        # 5) (A) GitHub Actions가 사용할 최신본 newsletter.py 갱신
        upload_file_to_github(
            SCRIPT_FILENAME,   # path_in_repo: 'newsletter.py'
            script_content,
            commit_message=f"Update newsletter latest script: {timestamp}",
        )

        # 6) (B) 타임스탬프 붙인 버전 파일을 추가로 저장
        upload_file_to_github(
            versioned_repo_path,   # path_in_repo: 'versions/newsletter_YYYY_MM_DD_HH_MM.py'
            script_content,
            commit_message=f"Archive newsletter script version: {timestamp}",
        )

        print(f"[Colab] GitHub에 최신본 newsletter.py와 버전 파일 {versioned_repo_path} 를 업로드했습니다.")
...
if IN_COLAB:
    try:
        ...
        if not os.path.exists(SCRIPT_FILENAME):
            print(f"[경고] Colab에 {SCRIPT_FILENAME} 파일이 없습니다.")
            print("현재 실행 중인 notebook 내용을 파일로 저장한 뒤 다시 실행해주세요.")
            raise FileNotFoundError(f"{SCRIPT_FILENAME} not found in Colab environment")

        # 1) KST 타임스탬프 생성
        KST = timezone(timedelta(hours=9))
        now_kst = datetime.now(KST)
        timestamp = now_kst.strftime("%Y_%m_%d_%H_%M")

        versioned_filename = f"newsletter_{timestamp}.py"
        versioned_repo_path = f"versions/{versioned_filename}"
        ...
        print(f"[Colab] 최신본 + 버전 파일 업로드 완료 → {versioned_repo_path}")
...
if IN_COLAB:
    ...
    # 현재 시간 (KST 기준)
    now_kst = datetime.now(KST)
    timestamp_str = now_kst.strftime("%Y_%m_%d_%H_%M")
    ...
    # 업로드할 파일 경로
    notebook_path = "/content/newsletter.py"  # Colab에서 현재 노트북 경로
    ...
    try:
        ...
        # 2️⃣ 버전 폴더에 날짜별 파일 업로드 (versions/newsletter_2025_12_12_13_50.py)
        ...
        if response_version.status_code in [200, 201]:
            ...
            print(f"   응답: {response_version.text}")
        ...
        print("   Colab에서 이 노트북을 다운로드하여 업로드해주세요.")
    except Exception as e:
...
if IN_COLAB:
    ...
    try:
        ...
        if not GITHUB_TOKEN:
            ...
            # 🔥 핵심: Colab 노트북을 .py로 변환
            print("📝 노트북을 Python 파일로 변환 중...")
            
            # Colab 노트북 다운로드 및 변환
            get_ipython().system('jupyter nbconvert --to script *.ipynb --output temp_newsletter')
            
            # 변환된 파일 읽기
            import glob
            py_files = glob.glob("/content/temp_newsletter.py")
            
            if not py_files:
                # 대체 방법: 현재 셀의 코드 직접 가져오기
                print("⚠️ nbconvert 실패, 대체 방법 사용...")
                
                # IPython 히스토리에서 모든 코드 가져오기
                from IPython import get_ipython
                ip = get_ipython()
                
                # 모든 실행된 셀의 코드 수집
                all_code = []
                for cell_num in range(1, len(ip.user_ns.get('In', [])) + 1):
                    cell_code = ip.user_ns.get('In', {}).get(cell_num, '')
                    if cell_code.strip():
                        all_code.append(f"# Cell {cell_num}\n{cell_code}\n")
            ...
            if file_size_mb > 1:
                print(f"⚠️ 파일이 너무 큽니다 ({file_size_mb:.2f}MB).")
            elif len(file_content.strip()) == 0:
                print("❌ 파일 내용이 비어있습니다. 변환 실패.")
            else:
                encoded_content = base64.b64encode(file_content.encode('utf-8')).decode('utf-8')
                ...
                if response_version.status_code in [200, 201]:
                    ...
                    print(f"❌ 버전 파일 업로드 실패: {response_version.status_code}")
                    print(f"   응답: {response_version.text[:200]}")
